...
def upgrade() -> None:
    for table in TABLES:
        op.add_column(table, sa.Column("process_state", sa.String(64)))
        op.add_column(table, sa.Column("host", sa.String(255)))
        op.add_column(table, sa.Column("retry_times", sa.Integer()))
        op.add_column(table, sa.Column("max_retry_times", sa.Integer()))
        op.add_column(table, sa.Column("started_at", sa.DateTime(timezone=True)))
        op.add_column(table, sa.Column("finished_at", sa.DateTime(timezone=True)))
        op.add_column(table, sa.Column("duration_seconds", sa.Float()))
        op.add_column(table, sa.Column("last_synced_at", sa.DateTime(timezone=True)))
