...
未保存不等于不存在。它显示项目下一个可保存版本号，并有一对一 Workspace，可以运行、展示和参与
对比；继续修改并运行仍复用这一个 Version/Workspace，只新增 Attempt，参数和当前结果更新。
...
批量提交只创建内部待保存记录，不会提前占用公开版本号。每个成功结果完成校验和指标收集后，才在
项目锁内取得当前未保存版本显示的号码，并把当前版本推进一位；失败、排队中和自动保存失败的任务
都不产生版本号。版本号是只增不减的外部标识，历史空号不会补用。并发项按结果成功保存的
顺序编号，不按 `items` 的位置提前保留号码；应使用 `client_id` 和 `workspace_id` 识别具体项。批量
结果保存期间，已打开页面上的当前未保存版本号可能随新版本落库而递增，但它的 Workspace、参数和
Attempt 不会因此更换。主动删除已保存版本会永久留下空号，系统不会重编号或复用仍可能被外部引用
的历史版本号。升级旧数据时同样保留所有已保存版本的原编号，只清除未成功批量记录的内部占号。
