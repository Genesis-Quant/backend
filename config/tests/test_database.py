...
def test_sqlalchemy_database_url_uses_external_credentials(monkeypatch):
    monkeypatch.setenv("POSTGRES_USER", "arena")
    monkeypatch.setenv("POSTGRES_PASSWORD", "secret")

    url = database.sqlalchemy_database_url("postgresql://db.example:5433/dolphinscheduler")

    assert url.drivername == "postgresql+psycopg"
    assert url.host == "db.example"
    assert url.port == 5433
    assert url.username == "arena"
