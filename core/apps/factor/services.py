...
def create_factor_version(session: Session, user_id: int, project_id: int, workflow_instance_id: int, remark: str, metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    validate_metric_dimensions(run.payload, metrics)
    next_version = (session.scalar(select(func.max(FactorVersion.version)).where(FactorVersion.project_id == project.id)) or 0) + 1
    version = FactorVersion(project_id=project.id, workflow_instance_id=workflow.workflow_instance_id, version=next_version, remark=remark, parameters=run.payload, metrics=metrics)
    session.add(version)
    run.saved = True
    project.updated_at = utc_now()
    session.commit()
