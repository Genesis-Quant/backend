...
SERVER_INSTRUCTIONS = (
    ...
    "FactorQuery, read arena://docs/overview/dsl. Prefer Python DSL for every newly constructed or edited query unless the user "
    "explicitly requests JSON or an existing record already selects JSON. Call compile_python_dsl with the matching application "
    "context and fix every returned error before run_*. Python operators use the exact TYPE.category.operator hierarchy, such as "
    "DIRECT.binary.div; flat aliases are invalid. When an operator accepts nested DSL, keep single-use intermediate nodes nested "
    ...
    "and DolphinDB signatures instead of guessing. "
    "For Factor analysis, never define stock_pool_member in the dataset DSL; it may reference that managed BOOL name. The Backend "
    "injects true for the full market, or the selected index membership definition and filter for a managed index pool. "
    "Business results are in structuredContent.result. Follow "
